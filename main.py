...
clock = time.Clock()
display.set_caption('Шутанок')
background = transform.scale(image.load('backs.jpg'),(1000,700))

mixer.init()
mixer.music.load('backmusic.mp3')
mixer.music.play()
#fire_music = mixer.Sound('vistrel.mp3')
font.init()
...
class Player(Gamer):
    def control(self):
        ...
        if buttons[K_d] and self.rect.x<920:
            self.rect.x += self.step
    def fire(self):
        bullet = Bullet('poco.jpg',self.rect.x+50,self.rect.y,15,20,15)
        bullets.add(bullet)
class Enemy(Gamer):
    def update(self):
...
game = True 
finish = False
Tema = Player('soldat.png',460,600,100,100,10)
bullets = sprite.Group()
asteroids = sprite.Group()
for i in range(1,3):
    asteroid = Enemy_1('bomb.png',randint(80,920),0,80,80,randint(1,3))
    asteroids.add(asteroid)
monsters = sprite.Group()
for i in range(1,5):
    monster = Enemy('wrag.jpg',randint(0,920),0,80,80,randint(2,7))
...
while game:
    for e in event.get():
        if e.type == QUIT:
            ...
            if e.key == K_l and kol_billets < 12 and r == False:
                kol_billets +=1
                #fire_music.play()
                Tema.fire()
            if kol_billets >= 12 and r == False:
                start_t = timer()
                r = True 
    if finish != True:
        ...
        for i in babah:
            score += 1
            monster = Enemy('wrag.jpg',randint(80,920),0,80,80,randint(1,5))
            monsters.add(monster)
        if score >=50:
            finish = True
        display.update()
